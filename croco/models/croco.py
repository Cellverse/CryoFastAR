...
from models.blocks import Block, DecoderBlock, PatchEmbed, BertBlock
...
class CroCoNet(nn.Module):
    ...
    def __init__(self,
        ...
        # decoder
        if dec_type in ['croco', 'mem']:
            self._set_decoder(enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, norm_im2_in_dec)
        elif dec_type == 'bert':
            self._set_bert_decoder(enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, dec_num_views)
        # prediction head
    ...
    def _set_bert_decoder(self, enc_embed_dim, dec_embed_dim, dec_num_heads, dec_depth, mlp_ratio, norm_layer, dec_num_views):

        self.dec_depth = dec_depth
        self.dec_embed_dim = dec_embed_dim
        self.decoder_embed = nn.Linear(enc_embed_dim, dec_embed_dim, bias=True)
        self.num_views = dec_num_views

        self.norm0 = norm_layer(dec_embed_dim)
        self.dec_blocks = nn.ModuleList([
            BertBlock(dec_embed_dim, dec_num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=norm_layer, rope=self.rope, num_seqs=dec_num_views)
            for i in range(dec_depth)])
