...
def is_symmetrized(gt1, gt2):
    ...
    for i in range(0, len(x), 2):
        ok = ok and (torch.equal(x[i], y[i+1])) and (torch.equal(x[i+1], y[i]))
    return ok
...
def interleave(tensors):
    # interleave tensors
    view_num = len(tensors)

    results = [[] for _ in range(view_num)]
    for i in range(view_num):
        for j in range(view_num):
            results[(i + j) % view_num].append(tensors[j])
    
    for i in range(view_num):
        results[i] = torch.stack(results[i], dim=1).flatten(0, 1)
