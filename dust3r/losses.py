...
from dust3r.utils.fft import iht2_center, ht2_center

class ImageL2Loss(nn.Module):
    '''
    a tool function to calculate the l2 loss of two images with / without (inverse) 2D Hartley transform.
    If applying Hartly transform, the shape of input images should be (B, H, W) or (B, 1, H, W)
    '''
    ...
    def get_name(self):
        return f'{type(self).__name__}({self.transform})'

    def forward(self, gt, pred):

        if self.transform == 'ht2':
            assert len(gt.shape) == 3 or len(gt.shape) == 4, ValueError("unacceptable input shape of ground truth.")
            assert gt.shape == pred.shape, ValueError(f"unmatched shape of ground truth and prediction, {gt.shape} and {pred.shape}.")
            gt = ht2_center(gt)
            pred = ht2_center(pred)
        elif self.transform == 'iht2':
            assert len(gt.shape) == 3 or len(gt.shape) == 4, ValueError("unacceptable input shape of ground truth.")
            assert gt.shape == pred.shape, ValueError(f"unmatched shape of ground truth and prediction, {gt.shape} and {pred.shape}.")
            gt = iht2_center(gt)
            pred = iht2_center(pred)
