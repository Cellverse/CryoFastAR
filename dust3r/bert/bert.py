...
import copy
import json
from .layers import LayerNorm, MultiHeadAttentionLayer, PositionWiseFeedForward, activations
...
class BertEmbeddings(nn.Module):
    ...
    """
    def __init__(self, hidden_size, segment_vocab_size, drop_rate):
    ...
    def forward(self, embeddings, segment_ids=None):
        seq_length = embeddings.size(1)

        if segment_ids is None:
            segment_ids = torch.zeros_like(embeddings)
...
class BERT(Transformer):
    ...
    def __init__(
        ...
        self.embeddings = BertEmbeddings(self.hidden_size, self.segment_vocab_size, self.dropout_rate)
    ...
    def forward(self, embeddings, segment_ids=None, attention_mask=None, output_all_encoded_layers=False):
        """
        ...
        # 执行embedding
        hidden_states = self.embeddings(embeddings, segment_ids)
        # 执行encoder
